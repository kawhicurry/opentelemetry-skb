...
struct ksyms *load_kallsyms_local(void)
{
    ...
    while (fgets(buf, sizeof(buf), f))
    {
        ...
        // !!! rewrite libbpf_ensure_mem
        ksyms->syms = (struct ksym *)realloc(ksyms->syms, (ksyms->sym_cap + 1) * sizeof(struct ksym));
        ksyms->sym_cap += 1;
        ret = 0;
        if (ret)
            goto error;
    ...
    if (!ksyms->sym_cnt)
    {
        fprintf(stderr, "Error: Need permission to read /proc/kallsyms!\n");
        exit(-1);
    }
...
parse_build_id_buf(const void *note_start, Elf32_Word note_size, char *build_id)
{
    ...
    while (note_offs + sizeof(Elf32_Nhdr) < note_size)
    {
        ...
        {
            memcpy(build_id, note_start + note_offs + ALIGN(sizeof("GNU"), 4) + sizeof(Elf32_Nhdr), nhdr->n_descsz);
